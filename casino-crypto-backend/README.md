...
- Mines board has `25` cells and allows `1..24` mines.
- Multiplier is dynamic by `mineCount` and number of safe reveals.
- Global anti-abuse normalization baseline: `x1` at (`mineCount = 1`, `safeReveals = 3`), and equivalent risk points
  (e.g. `mineCount = 3`, `safeReveals = 1`) align to `x1`.

## Roulette rounds + websocket

- Round states: `OPEN`, `CLOSED`, `SPINNING`, `SETTLED` (and `CANCELLED` for operational fallback).
- Automatic phase timings: `BETTING=10s`, `SPINNING=3s`, `RESULT=5s`.
- Shared round state is persisted in Redis so multi-instance deployments stay in sync.
...
- Websocket endpoint streams round state transitions and stake totals for realtime clients via Redis pub/sub fanout.

### Roulette probability model (custom 15-slot wheel)

This roulette mode uses `15` equally likely outcomes:

- `1..14`: colored numbers (`7` red + `7` black)
- `15`: green
- `BAIT` wins when outcome is neighbor of green (`14` or `1`)
...
| --- | --- | --- | --- | --- | --- | --- |
| `RED`, `BLACK` | 7 | `7/15` (46.6667%) | `2x` | `14/15` (93.3333%) | `-1/15` (-6.6667%) | `1/15` (6.6667%) |
| `GREEN` | 1 | `1/15` (6.6667%) | `14x` | `14/15` (93.3333%) | `-1/15` (-6.6667%) | `1/15` (6.6667%) |
| `BAIT` (`left/right` of green) | 2 | `2/15` (13.3333%) | `7x` | `14/15` (93.3333%) | `-1/15` (-6.6667%) | `1/15` (6.6667%) |
