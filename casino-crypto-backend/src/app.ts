...
import { fairRoutes } from "./modules/fair/routes";
...
export const buildApp = (): FastifyInstance => {
  ...
  app.register(rateLimit, {
    global: true,
    max: RATE_LIMIT_POLICY.general.max,
    timeWindow: RATE_LIMIT_POLICY.general.timeWindow,
    redis
  });
  ...
  app.register(swagger, {
    openapi: {
      ...
      tags: [
        ...
        { name: "fair", description: "Provably fair verification for roulette and mines" },
  ...
  app.register(fairRoutes, { prefix: "/api/v1" });
  ...
  app.addHook("onReady", () => {
    void ensureRedisConnections().catch((error) => {
      app.log.error({ err: error }, "Redis bootstrap failed");
    });
  });
