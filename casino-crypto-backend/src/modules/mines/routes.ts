...
export const minesRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  const startGameHandler = async (request: any, reply: any) => {
  ...
  const minesBetRouteOptions = {
    preHandler: [requireAuth, requireIdempotencyKey],
    config: {
      rateLimit: RATE_LIMIT_POLICY.minesBet
    }
  };

  fastify.post("/games", minesBetRouteOptions, startGameHandler);
  fastify.post("/bet", minesBetRouteOptions, startGameHandler);
