...
import { RATE_LIMIT_POLICY } from "../../core/rate-limit";
...
const toRoundResponse = (round: RouletteRoundState) => ({
  ...
  winningIsBait: round.winningIsBait,
...
export const rouletteRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.addHook("onReady", () => {
    void websocketHub.start().catch((error) => {
      fastify.log.error({ err: error }, "Roulette websocket hub bootstrap failed");
    });
    setRouletteBroadcaster(websocketHub);
    void startRouletteRoundWorker(fastify.log).catch((error) => {
      fastify.log.error({ err: error }, "Roulette worker bootstrap failed");
    });
  });
  ...
  const placeBetHandler = async (request: any, reply: any) => {
  ...
  const placeBetRouteOptions = {
    preHandler: [requireAuth, requireIdempotencyKey],
    config: {
      rateLimit: RATE_LIMIT_POLICY.rouletteBet
    }
  };

  fastify.post("/bets", placeBetRouteOptions, placeBetHandler);
  fastify.post("/bet", placeBetRouteOptions, placeBetHandler);
