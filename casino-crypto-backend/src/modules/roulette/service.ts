...
import { drawRouletteProvablyFairOutcome } from "../fairness/roulette";
...
const ROUND_BETTING_MS = 10_000;
const ROUND_CLOSE_TO_SPIN_MS = 0;
const ROUND_SPINNING_MS = 3_000;
const ROUND_RESULT_MS = 5_000;
const WORKER_TICK_MS = 1_000;
const WORKER_LOCK_KEY = "roulette:engine:worker-lock";
const WORKER_INSTANCE_ID = randomUUID();
const WORKER_LOCK_TTL_MS = WORKER_TICK_MS * 3;
...
const tryAcquireWorkerLease = async (): Promise<boolean> => {
  const owner = await redis.get(WORKER_LOCK_KEY);
  if (owner === WORKER_INSTANCE_ID) {
    await redis.pexpire(WORKER_LOCK_KEY, WORKER_LOCK_TTL_MS);
    return true;
  }

  const acquired = await redis.set(WORKER_LOCK_KEY, WORKER_INSTANCE_ID, "PX", WORKER_LOCK_TTL_MS, "NX");
  return acquired === "OK";
};

const toWinningIsBait = (winningNumber: number | null): boolean | null =>
  typeof winningNumber === "number" ? isRouletteBaitNumber(winningNumber) : null;
...
const toRoundState = (round: RouletteRound): RouletteRoundState => ({
  ...
  winningIsBait: toWinningIsBait(round.winningNumber ?? null),
  totalStakedAtomic: round.totalStakedAtomic,
  totalPayoutAtomic: round.totalPayoutAtomic
});

const persistRoundStateInRedis = (round: RouletteRound): void => {
  void persistRouletteRoundState({
    roundId: round.id,
    roundNumber: round.roundNumber,
    currency: round.currency,
    status: round.status,
    phase: toRoulettePhase(round.status),
...
const toRoundStateFromRedis = (raw: NonNullable<Awaited<ReturnType<typeof getPersistedRouletteRoundState>>>): RouletteRoundState => ({
  id: raw.roundId,
  roundNumber: raw.roundNumber,
  currency: raw.currency,
  status: raw.status,
  openAt: new Date(raw.openAt),
  betsCloseAt: new Date(raw.betsCloseAt),
  spinStartsAt: new Date(raw.spinStartsAt),
  settleAt: new Date(raw.settleAt),
  winningNumber: raw.winningNumber,
  winningColor: raw.winningColor,
  winningIsBait: raw.winningIsBait,
  totalStakedAtomic: BigInt(raw.totalStakedAtomic),
  totalPayoutAtomic: BigInt(raw.totalPayoutAtomic)
});

const emitRoundEvent = (round: RouletteRound): void => {
  persistRoundStateInRedis(round);
...
const createRoundForCurrencyTx = async (
  ...
  const betsCloseAt = new Date(openAt.getTime() + ROUND_BETTING_MS);
  const spinStartsAt = new Date(betsCloseAt.getTime() + ROUND_CLOSE_TO_SPIN_MS);
  const settleAt = new Date(spinStartsAt.getTime() + ROUND_SPINNING_MS);
...
const settleRoundOnce = async (now: Date): Promise<RouletteRound | null> =>
  prisma.$transaction(async (tx) => {
    ...
    const fairOutcome = await drawRouletteProvablyFairOutcome({
      minNumber: ROULETTE_MIN_NUMBER,
      maxNumber: ROULETTE_MAX_NUMBER,
      roundId: round.id,
      currency: round.currency
    });
    const winningNumber = fairOutcome.winningNumber;
    const winningColor = getRouletteColor(winningNumber);
    const winningIsBait = isRouletteBaitNumber(winningNumber);
    ...
    for (const bet of round.bets as RouletteBetWithRelations[]) {
      ...
      void settleActiveRouletteBet(round.id, bet.id).catch(() => undefined);
    }
    ...
    const updatedRound = await tx.rouletteRound.update({
      ...
      data: {
        ...
        winningColor,
        settledAt: now,
        settleAt: new Date(now.getTime() + ROUND_RESULT_MS),
        totalPayoutAtomic
      }
    });
    ...
    await tx.outboxEvent.create({
      data: {
        ...
        payload: {
          ...
          winningColor,
          winningIsBait,
          fairnessHash: fairOutcome.proof.hash,
          fairnessNonce: fairOutcome.proof.nonce,
          fairnessServerSeedHash: fairOutcome.proof.serverSeedHash,
          fairnessClientSeed: fairOutcome.proof.clientSeed,
          totalStakedAtomic: round.totalStakedAtomic.toString(),
  ...
  for (let i = 0; i < 10; i += 1) {
    ...
    void enqueueAuditEvent({
      ...
      metadata: {
        ...
        winningIsBait: toWinningIsBait(settled.winningNumber ?? null),
...
const ensureOpenRounds = async (): Promise<void> => {
  for (const currency of SUPPORTED_CURRENCIES) {
    ...
    if (!active || active.status === RouletteRoundStatus.CANCELLED) {
...
export const getCurrentRouletteRound = async (currency: Currency): Promise<RouletteRoundState> => {
  const cached = await getPersistedRouletteRoundState(currency);
  if (cached) {
    const cachedRound = toRoundStateFromRedis(cached);
    const now = Date.now();
    const isExpiredResultPhase =
      cachedRound.status === RouletteRoundStatus.SETTLED && cachedRound.settleAt.getTime() <= now;

    if (!isExpiredResultPhase && cachedRound.status !== RouletteRoundStatus.CANCELLED) {
      return cachedRound;
    }
  }
